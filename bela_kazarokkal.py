...
belaek = data.iloc[:belaek_count]
belaek_names = belaek[index_column]
belaek = belaek.drop([label_column, index_column], axis=1)

data = data.iloc[belaek_count:]
...
if do_tsne:
    ...
    train_and_belaek = np.concatenate((train_X, belaek))
    reduced = embedder.fit_transform(train_and_belaek)
    for i, l in enumerate(labels):
        plt.scatter(reduced[:-len(belaek)][train_y==i, 0], reduced[:-len(belaek)][train_y==i, 1], label=l)
    print("unimplemented Belaek vis")
...
for i, name in enumerate(belaek_names):
    bela = belaek[i:i+1]
    print(name, "haplogroup:", le.inverse_transform(classifier.predict(bela)[0]))

    bela_y = classifier.predict_proba(bela)

    logg(le.inverse_transform(range(np.max(train_y)+1)))
    np.set_printoptions(suppress=True)
    print(name, "haplogroup probs:", bela_y)
